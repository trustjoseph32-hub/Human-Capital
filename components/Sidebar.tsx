...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const navItems = [
    ...
    { id: 'tasks', label: 'Реестр ТЗ (001-030)', icon: Database },
    { id: 'consilium', label: 'ИИ-Консилиум', icon: Binary },
    { id: 'monograph', label: 'Текст Рукописи', icon: ScrollText },
    { id: 'sources', label: 'База Источников', icon: BookOpenCheck },
    { id: 'terminal', label: 'Core Runtime', icon: Terminal },
  ];

  return (
    <div className="w-72 bg-[#050608] text-slate-400 flex flex-col h-screen fixed left-0 top-0 border-r border-white/5">
      ...
      <div className="p-6 border-t border-white/5 bg-[#08090c]">
        <div className="flex items-center justify-between text-[10px] font-black text-slate-500 uppercase mb-4 tracking-tighter">
          ...
          <span className="text-amber-500/50">2025-2026</span>
        </div>
        <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-white/5 rounded-lg transition-colors group">
          <Settings size={16} className="group-hover:rotate-90 transition-transform duration-500" />
          <span className="font-bold text-[10px] uppercase tracking-widest">Система</span>
        </button>
      </div>
    </div>
  );
